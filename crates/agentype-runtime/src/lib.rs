...
impl ResolvedExecutionEnvironment {
    ...
    pub fn safety(&self) -> FrozenExecutionSafety {
        // SAFETY: attempt_isolation was loaded directly from the authoritative
        // ExecutionTargetConfig during resolve_execution_environment.
        unsafe {
...
pub fn resolve_execution_environment(
    ...
    match mode {
        ...
        ExecutionResolutionMode::DirectUnconfigured => Ok(ResolvedExecutionEnvironment {
...
mod tests {
    ...
    fn valid_target_and_profile_resolve_isolation() {
        ...
        assert_eq!(
            env.safety(),
            FrozenExecutionSafety::for_testing("local-b", "isolated-writer", true)
        );
    }

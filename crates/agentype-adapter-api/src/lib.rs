...
mod deadline;
pub mod worker_protocol_v01;

pub use deadline::{AdapterDeadline, AdapterOperation, DeadlineConfigError};

use agentype_core::{ExecutionId, ExecutionState, RequestId, WorkspaceMode};
...
/// `EnvironmentStartRequest::from_launch` (or the worker-plane
/// `ExecutionRequest`) do not describe the same attempt identity.
...
mod tests {
    use super::*;
    use crate::worker_protocol_v01::{ExecutionRequest, RenderedWorkerPrompt};
    use agentype_core::{
        BatchId, CommittedContinuitySnapshot, IncarnationId, LeaseEpoch, LeaseId, LogicalAgentId,
        TaskId, WorkstreamId,
    };
